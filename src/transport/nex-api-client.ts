...
import { SESSIONS_ROOT } from './routes.js';

/** Default NexBrowser OpenAPI address used when NEX_API_HOST/--api-host is absent. 未提供 NEX_API_HOST/--api-host 时使用的默认 NexBrowser OpenAPI 地址。 */
...
/**
 * Sends authenticated, timeout-protected requests to NexBrowser OpenAPI.
 * 对 NexBrowser OpenAPI 发起经过认证和超时保护的请求。
 */
export class NexApiClient {
  /** Sent as X-Nex-Client-Id on every request so the server scopes session ownership per client. 随每个请求以 X-Nex-Client-Id 头上报，服务端据此按客户端隔离会话归属。 */
  ...
  async request<T = unknown>(path: string, options: RequestInit = {}): Promise<NexApiResponse<T>> {
    ...
    try {
      ...
      headers.set('X-Nex-Client-Id', this.clientId);
  ...
  async close(): Promise<void> {
    await this.request(SESSIONS_ROOT, { method: 'DELETE' }).catch(() => undefined);
