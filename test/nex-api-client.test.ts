...
describe('NexApiClient', () => {
  it('uses the local OpenAPI host by default', () => {
  ...
  it('keeps each client identity stable and adds authenticated request headers', async () => {
    ...
    expect(firstHeaders.get('token')).toBe('test-token');
    expect(firstHeaders.get('X-Nex-MCP-Client')).toBe(secondHeaders.get('X-Nex-MCP-Client'));
    expect(firstHeaders.get('X-Nex-MCP-Client')).not.toBe(thirdHeaders.get('X-Nex-MCP-Client'));
  ...
  it('does not let callers replace authentication headers', async () => {
    ...
    await client.request('/test', {
      headers: { token: 'attacker-token', 'X-Nex-MCP-Client': 'attacker-client' }
    });
    const headers = new Headers(fetchMock.mock.calls.at(0)?.[1]?.headers);
    expect(headers.get('token')).toBe('trusted-token');
    expect(headers.get('X-Nex-MCP-Client')).toBe(client.clientId);
  });
