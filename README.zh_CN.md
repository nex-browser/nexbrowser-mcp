...
- 浏览器自动化：连接环境、查看页面、生成快照或截图、操作页面元素，以及读取 console 和
  network 记录。
- 会话隔离：每个 MCP 进程都有独立的 `X-Nex-MCP-Client` 标识，浏览器 session 和活动标签
  不会在不同 MCP 客户端之间共享。
...
2. 开启**本地 OpenAPI**。服务仅监听 `127.0.0.1`，默认端口为 `45536`。
...
| 变量             | 是否必填 | 默认值                   | 说明                                      |
| ---------------- | -------- | ------------------------ | ----------------------------------------- |
| `NEX_API_HOST`   | 否       | `http://127.0.0.1:45536` | NexBrowser 本地 OpenAPI 基地址。          |
